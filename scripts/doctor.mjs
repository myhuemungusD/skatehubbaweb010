...
const fail = (m) => {
  console.error("DEPLOY BLOCKED:", m);
  process.exit(1);
};
...
if (!/root:\s*["']client["']/.test(viteCfg))
  fail("vite.config.ts must set root:'client'");
ok("vite root set");
...
const meta = {
  build: process.env.REPL_SLUG || "local",
